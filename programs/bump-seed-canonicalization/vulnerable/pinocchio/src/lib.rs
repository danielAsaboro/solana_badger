...
fn withdraw(accounts: &[AccountView], bump: u8) -> ProgramResult {
    ...
    // VULNERABILITY: Uses user-supplied bump without verifying it's the canonical one
    let user_bytes = user.address().as_ref();
    let seeds: &[&[u8]] = &[b"vault", user_bytes, &[bump]];

    // Verify the PDA with the provided bump
    ...
    // Read vault state but DON'T verify bump matches stored canonical bump
    let vault_data = vault.try_borrow()?;
    let _vault_state = VaultState::deserialize(&vault_data)
        .map_err(|_| ProgramError::InvalidAccountData)?;

    // VULNERABILITY: An attacker could use a non-canonical bump to create
    // an alternate PDA with the same seeds, potentially bypassing security checks
