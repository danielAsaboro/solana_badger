...
impl Admin {
    ...
    /// SECURE: Deserialize with discriminator validation
    ///
    /// This checks that data[0] == Admin::DISCRIMINATOR before deserializing.
    /// This prevents User accounts from being deserialized as Admin!
    pub fn deserialize(data: &[u8]) -> Result<AdminData, &'static str> {
        if data.len() < Self::LEN {
            return Err("Account data too small");
        }

        // SECURITY: Check discriminator BEFORE deserializing
